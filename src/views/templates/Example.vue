<template>
  <el-main>
    <el-form :model="form" :rules="rules" ref="ruleFormRef" label-position="top">
      ...
      <el-row :gutter="30">
        <Input :label="'名稱'" v-model="form.name" prop=""
          :span="6" :clearable="true" :multiple="false" :disabled="false" :placeholder="'請輸入名稱'" />
    ...
    <CommonSearchBarButton :buttons="[
      { show: true, type: 'primary', func: () => searchbar_query(), label: '查詢' },
      { show: true, type: 'danger', func: () => searchbar_clear(), label: '清除' },
      { show: true, type: 'success', func: () => dialogHandler(true, 'add', '新增', null), label: '派車完成' },
    ]" />
